...
function getInitialDB(): MockDB {
  return {
    folders: [
      { id: 'fold_root_docs', name: 'Documents', parentId: null, createdAt: now(), trashedAt: null },
      { id: 'fold_root_media', name: 'Media', parentId: null, createdAt: now(), trashedAt: null },
    ],
    files: [
      {
        id: 'file_demo_1',
...
const filesService: FileService = {
  ...
  async purge(fileId: ID): Promise<void> {
    ...
    db.usage.usedBytes = Math.max(0, db.usage.usedBytes - db.files[idx].sizeBytes)
...
const foldersService: FolderService = {
  ...
  async create(name: string, parentId: ID | null): Promise<FolderItem> {
    requireAuth()
    const db = loadDB()
    const folder: FolderItem = { id: randomId('fold'), name, parentId, createdAt: now(), trashedAt: null }
  ...
  async delete(folderId: ID): Promise<void> {
    ...
    db.files.filter((f) => f.folderId === folderId).forEach((file) => {
      file.trashedAt = now()
    })
    saveDB(db)
  },
  ...
  async purge(folderId: ID): Promise<void> {
    requireAuth()
    const db = loadDB()
    const fileIds = new Set(db.files.filter((f) => f.folderId === folderId).map((f) => f.id))
